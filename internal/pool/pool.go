...
type Status struct {
	...
	// SoftRateModel 触发本次模型级软冷却（429 code=6004）的模型名；
	// 非空表示该冷却只针对此模型，其他模型请求仍可用（见 healthyForModel）。
...
type entry struct {
	...
	// sessionDeadFails 连续 12153（ErrSessionDead）计数。12153 在真实环境会被临时性
	// 触发（网络抖动/上游闪断/refresh 竞态），一次失败就永久禁用太粗暴——连续达到
	// 阈值才判死。运行态语义（不持久化，与 inFlight 同语义）：重启清零可接受——
	// 重启后首个 keepalive 成功即清计数，误判号不会因历史累积被继续追杀。
	sessionDeadFails int

	// softRateModel 触发本次**模型级**软冷却的模型名（仅 429 code=6004 且能解析出
	// 上游"将在 … 重置"时间时记录）。非空且与请求模型不同时，该账号对本请求豁免冷却
	// ——模型限流不代表账号在其他模型下不可用。运行态（不持久化，重启清零）。
...
// 模型下不可用。空 reqModel / 未记录模型 / 同模型 → 与 healthy 一致。
func (e *entry) healthyForModel(now time.Time, reqModel string) bool {
	if !e.healthy(now) && reqModel != "" && e.softRateModel != "" &&
		...
		// 非 healthy 但属于可豁免场景：仍受 disabled 与**生效中的**熔断约束
		// （熔断/禁用是账号级事实，模型豁免不覆盖）。
		// 注意只判"熔断尚未到期"，不能判 breakerUntil.IsZero()：已过期的熔断
		// 时间戳非零，那样写会把已恢复的账号误判为不可用。
		if e.disabled {
			return false
		}
		return e.breakerUntil.IsZero() || !now.Before(e.breakerUntil)
	}
...
// 会误杀健康账号。3 次连续才判死：容忍偶发抖动，又不会让真正的死 session 留在池里
// 反复被选中。
...
// 账号」进行模型豁免——请求模型与其 trigger 模型不同时视为可用。
...
func (p *Pool) pick(tried map[string]bool, reqModel string) *auth.Auth {
	...
	// 健康口径：reqModel 非空时启用模型级豁免（6004 冷却中的账号换个模型仍可用）。
	// 注意豁免只作用于 normal 选号；全冷却兜底不参与（兜底本就是"无 direct 可用"
	// 时的降级，切模型可用性已在 normal 阶段体现）。
...
func (p *Pool) Cooldown(uid string, kind CoolKind, d time.Duration, reason string) {
	...
	if e, ok := p.byUID[uid]; ok {
		...
		e.coolKind = kind
		e.reason = reason
		// 账号级冷却入口：清空模型豁免痕迹。否则上一次 6004 模型级冷却留下的
		// softRateModel 会让换模型的请求错误绕过本次**账号级**冷却。
		e.softRateModel = ""
...
//
// 区别于 Cooldown：当上游 code=6004 明说「将在 … 重置」时，把冷却截止精确设为
...
//
// 熔断信号照旧喂入（冷却与熔断正交）；softStreak 仍递增——解析时间的冷却不参与
// 指数退避，但计数照常累加，后续无时间的 6004 从当前 streak 继续退避。
...
// softRateMaxOr 返回软冷却退避封顶；未注入（<=0）时按默认值。
// 调用方必须已持有 p.mu（与 softDurationLocked 同约定）。
...
// ModelSoftCooldown 查询账号当前的模型级冷却信息（供 /status 与测试观测）。
// 返回 (触发模型, 是否处于模型级软冷却)。model 为空表示非模型级冷却。
func (p *Pool) ModelSoftCooldown(uid string) (string, bool) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	e, ok := p.byUID[uid]
	if !ok {
	...
	return e.softRateModel, e.softRateModel != ""
}
...
//
// 为什么改：旧行为一次 12153 即 Disable 且无复活路径，但 12153 会被临时性触发
// （网络抖动/上游闪断/refresh 竞态），一次失败就永久杀号会误杀健康账号。
// 改为连续 sessionDeadThreshold 次才禁用：计数 +1，达到阈值 → Disable 并清计数；
...
func (p *Pool) reviveCoolingLocked(e *entry, credits int64) {
	...
	e.reason = ""
	e.softStreak = 0
	// 冷却域一并清空：模型豁免痕迹属于冷却域，解冻后不应残留
	// （否则下次账号级冷却期间换模型请求会被错误豁免）。
	e.softRateModel = ""
}
...
func (p *Pool) NoteSuccess(uid string) {
	...
	if e, ok := p.byUID[uid]; ok {
		...
		e.softStreak = 0
		// 成功证明 session 未死（与 ClearSessionDead 语义一致），清连续 12153 计数。
		e.sessionDeadFails = 0
		p.dirty.Store(true)
	}
}
